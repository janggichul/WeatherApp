...
export default function SignUpPageCode() {
  ...
  const [confirmPasswordMessage, seteconfirmPasswordMessage] =
    useState<string>('');
  ...
  const KAKAO_AUTH_URL: any = process.env.REACT_APP_KaKaoauthURL;

  const handleKakaoLogin = () => {
    window.location.href = KAKAO_AUTH_URL;
  };

  const handleEmailDuplicationButton = async () => {
    const docSnap = await getDoc(emailRef);
    console.log(docSnap);
    if (docSnap.exists()) {
      const array = docSnap.data().email;
      if (array.find((e: any) => e === email)) {
        Swal.fire({
  ...
  const writeUserData = async () => {
    const docSnap: any = await getDoc(emailRef);
  ...
  const handleNicknameDuplicationButton = async () => {
    const docSnap = await getDoc(emailRef);
    console.log(docSnap);
    if (docSnap.exists()) {
      const array = docSnap.data().name;
      if (array.find((e: any) => e === nickname)) {
        Swal.fire({
          ...
          <MemberInfo>
            <MemberInfoForm onSubmit={signUp}>
              ...
              <InfoDiv>
                ...
                <Duplication
                  type="button"
                  onClick={handleEmailDuplicationButton}
                >
                  중복확인
                </Duplication>
              </InfoDiv>
              ...
              <InfoDiv>
                ...
                <Duplication
                  type="button"
                  onClick={handleNicknameDuplicationButton}
                >
                  중복확인
                </Duplication>
              </InfoDiv>
